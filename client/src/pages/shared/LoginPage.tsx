...
// ─── Right panel stats (replace with real API data later) ─────────────────────
...
function LoginForm() {
  ...
  // Placeholder submit — will connect to Express /api/auth/login later
    ...
    <div className="flex flex-col items-center justify-center min-h-screen px-6 py-12 bg-white w-full max-w-lg mx-auto">
      ...
      <div className="w-full bg-white border border-slate-100 rounded-2xl shadow-sm p-6 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <a href="/forgot-password" className="text-sm text-amber-500 hover:text-amber-600 font-medium transition-colors">
            Forgot password?
          </a>
        </div>
      ...
      <a
        href="/"
        className="mt-6 text-sm text-slate-400 hover:text-slate-600 transition-colors flex items-center gap-1"
      >
        ← Back to home
      </a>
    </div>
  ...
  return (
    <div className="hidden lg:flex flex-col items-center justify-center bg-slate-800 px-12 py-16 text-white min-h-screen flex-1">
      ...
      <p className="text-slate-400 text-center text-sm leading-relaxed max-w-xs mb-12">
        Join customers finding reliable local service providers or offer
        your expertise as a verified artisan across the North West Region.
      </p>
      ...
      <div className="flex gap-4 w-full max-w-sm">
        {panelStats.map((stat) => (
          <div
            key={stat.label}
            className="flex-1 bg-slate-700 rounded-xl px-3 py-4 flex flex-col items-center text-center"
          >
            {stat.icon}
...
export default function LoginPage() {
  return (
    <div className="flex min-h-screen">
      {/* Left — form takes full width on mobile, half on large screens */}
