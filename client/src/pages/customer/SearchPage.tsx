...
// ─── Data — replace with GET /api/artisans?q=&service=&quarter= ───────────────
const allArtisans: Artisan[] = [
  { id: 1, initials: 'JS', name: 'John Smith',    service: 'Plumbing',   quarter: 'Mile 4',            distanceKm: 1.2, ratePerHour: 4500, rating: 4.9, reviews: 156, trustScore: 98, available: true,  avatarColor: 'bg-slate-500'   },
  { id: 2, initials: 'SJ', name: 'Sarah Johnson', service: 'Electrical', quarter: 'Commercial Avenue', distanceKm: 2.5, ratePerHour: 5500, rating: 4.8, reviews: 203, trustScore: 95, available: true,  avatarColor: 'bg-teal-500'    },
  { id: 3, initials: 'MB', name: 'Mike Brown',    service: 'Solar',      quarter: 'Bambili',           distanceKm: 3.1, ratePerHour: 6500, rating: 4.7, reviews: 89,  trustScore: 92, available: true,  avatarColor: 'bg-purple-500'  },
  { id: 4, initials: 'DL', name: 'David Lee',     service: 'Mechanic',   quarter: 'Mile 3',            distanceKm: 1.8, ratePerHour: 5000, rating: 4.9, reviews: 178, trustScore: 96, available: true,  avatarColor: 'bg-blue-500'    },
  { id: 5, initials: 'EW', name: 'Emma Wilson',   service: 'Laundry',    quarter: 'Ntarinkon',         distanceKm: 4.2, ratePerHour: 3500, rating: 4.6, reviews: 124, trustScore: 88, available: false, avatarColor: 'bg-rose-500'    },
  { id: 6, initials: 'RC', name: 'Robert Chen',   service: 'HVAC',       quarter: 'Up Station',        distanceKm: 2.9, ratePerHour: 6000, rating: 4.8, reviews: 142, trustScore: 94, available: true,  avatarColor: 'bg-amber-600'   },
  { id: 7, initials: 'AN', name: 'Alice Nkeng',   service: 'Tailoring',  quarter: 'Nkwen',             distanceKm: 5.1, ratePerHour: 2500, rating: 4.5, reviews: 67,  trustScore: 85, available: true,  avatarColor: 'bg-emerald-500' },
  { id: 8, initials: 'PF', name: 'Paul Fon',      service: 'Plumbing',   quarter: 'Bambui',            distanceKm: 6.3, ratePerHour: 4000, rating: 4.3, reviews: 45,  trustScore: 80, available: true,  avatarColor: 'bg-orange-500'  },
]
    ...
    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm hover:shadow-md hover:border-amber-200 transition-all flex flex-col gap-4">
      ...
      <div className="flex items-center gap-4 text-sm">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className={`font-extrabold ${trustColor(artisan.trustScore)}`}>{artisan.trustScore}%</span>
        </div>
      </div>
      ...
      <div className="flex gap-3 pt-1">
        ...
        <a href={`/artisan/${artisan.id}/book`}
...
function MapView() {
  return (
    <div className="bg-slate-100 rounded-2xl border border-slate-200 flex flex-col items-center justify-center min-h-[400px]">
      ...
      <p className="text-slate-400 text-sm text-center max-w-xs px-4">
        Leaflet + OpenStreetMap with Nominatim geocoding will pin artisan quarters here during backend integration.
      </p>
    </div>
  )
}
...
export default function SearchPage() {
  ...
  const filtered = useMemo(() => {
    return allArtisans.filter((a) => {
      ...
      const matchesQuarter = quarter === 'All Quarters' || a.quarter === quarter
      return matchesQuery && matchesService && matchesQuarter
    })
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-6">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              {/* Quarter filter — uses real NW Region data */}
              <div className="relative">
